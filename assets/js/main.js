...
(function ($) {
  ...
  function closeSideInfo() {
    ...
    $(".side-toggle").attr("aria-expanded", "false");
    $("body").removeClass("menu-open");
  }
  $(".side-info-close,.offcanvas-overlay").on("click", closeSideInfo);
  ...
  // Homepage calculator CTA — pointer spotlight (CSS vars)
  const homeCta = document.querySelector(".alwin-cta");
  if (
    homeCta &&
    window.matchMedia("(hover: hover) and (pointer: fine)").matches &&
    !window.matchMedia("(prefers-reduced-motion: reduce)").matches
  ) {
    homeCta.addEventListener("pointermove", function (e) {
      const box = homeCta.getBoundingClientRect();
      const x = ((e.clientX - box.left) / box.width) * 100;
      const y = ((e.clientY - box.top) / box.height) * 100;
      homeCta.style.setProperty("--cta-x", x + "%");
      homeCta.style.setProperty("--cta-y", y + "%");
    });
  }

  // cta text animation — skip static homepage calculator CTA
  const ctaAreas = document.querySelectorAll(".cta-area:not(.cta-area--static)");
  if (ctaAreas.length > 0) {
    ...
    const ctaBg = ctaAreas[0].querySelector(".area-bg");
    const ctaTitle = ctaAreas[0].querySelector(".section-title");
    if (ctaBg) {
      tl.to(ctaBg, { scale: "10", delay: 0.1, ease: "power2.in" });
    }
    if (ctaTitle) {
      tl.to(ctaTitle, { fontSize: document.documentElement.lang === "fa" ? "12vw" : "18vw", ease: "power2.in" }, "<");
    }
  }
  ...
  mm.add("(min-width: 1200px)", () => {
    if (document.querySelectorAll(".hero-area").length > 0) {
      ...
      const bigtextColor = isDarkMode ? "#FFFFFF" : "#33BAFF";
